...
- `@anthropic-ai/sdk`, model `claude-opus-4-8`, `thinking: {type:"adaptive"}`; structured outputs via `client.messages.parse` + `zodOutputFormat` (`@anthropic-ai/sdk/helpers/zod`); receipts parsed with vision/document content blocks (base64 image + PDF)
- Playwright (chromium) executing Claude **computer use** actions (`computer_20251124`, beta header `computer-use-2025-11-24`) — client-executed loop: screenshot → Claude action → Playwright performs it
...
app/
  ...
  api/upload/route.ts      # file upload → Claude vision parse → purchase records
...
lib/
  anthropic.ts             # client + model const
...
1. **Scaffold** — `create-next-app`, Tailwind, deps, `lib/anthropic.ts`, `lib/schemas.ts`, `lib/store.ts`.
...
4. **Matching** — `match.ts`: one Claude call per settlement with rules + purchase records → `{eligible, confidence, whyQualified, evidence:[recordIds], uncertainties[], estPayout}`. Only surface matches with evidence or plausible self-attestation.
